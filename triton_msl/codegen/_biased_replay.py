...
def lower_wide_rows(lowerer, plan):
    ...
    kb.raw_line("                int _p515_key_d1 = _p515_start_d + int(_p515_j_d1);")
    kb.raw_line("                float _p515_live_d1 = -INFINITY;")
    kb.raw_line(f"                if (_p515_key_d1 < {nk}) {{")
    score = emit_score("d1", "                    ", "_p515_key_d1")
    kb.raw_line(f"                    _p515_live_d1 = {score};")
    kb.raw_line("                }")
    kb.raw_line("                _p515_p_sum_d += exp(_p515_live_d1 - _p515_newmax_d);")
    kb.raw_line("            }")
    ...
    kb.raw_line("            float _p515_acc = 0.0f;")
    kb.raw_line("            float _p515_maximum_a = -INFINITY;")
    kb.raw_line(f"            for (int _p515_start_a = 0; _p515_start_a < {nk}; _p515_start_a += {cols}) {{")
    kb.raw_line("                float _p515_block_max_a = -INFINITY;")
    kb.raw_line(f"                for (uint _p515_j_a0 = 0u; _p515_j_a0 < {cols}u; ++_p515_j_a0) {{")
    kb.raw_line("                    int _p515_key_a0 = _p515_start_a + int(_p515_j_a0);")
    kb.raw_line("                    float _p515_live_a0 = -INFINITY;")
    kb.raw_line(f"                    if (_p515_key_a0 < {nk}) {{")
    score = emit_score("a0", "                        ", "_p515_key_a0")
    kb.raw_line(f"                        _p515_live_a0 = {score};")
    kb.raw_line("                    }")
    kb.raw_line("                    _p515_block_max_a = fmax(_p515_block_max_a, _p515_live_a0);")
    kb.raw_line("                }")
    kb.raw_line("                float _p515_newmax_a = fmax(_p515_maximum_a, _p515_block_max_a);")
    kb.raw_line("                float _p515_alpha_a = exp(_p515_maximum_a - _p515_newmax_a);")
    kb.raw_line("                _p515_acc *= _p515_alpha_a;")
    kb.raw_line(f"                for (uint _p515_j_a1 = 0u; _p515_j_a1 < {cols}u; ++_p515_j_a1) {{")
    kb.raw_line("                    int _p515_key_a1 = _p515_start_a + int(_p515_j_a1);")
    kb.raw_line(f"                    if (_p515_key_a1 < {nk}) {{")
    score = emit_score("a1", "                        ", "_p515_key_a1")
    kb.raw_line(
        f"                        _p515_acc += exp({score} - _p515_newmax_a) * "
        f"static_cast<float>({v}[uint(_p515_key_a1) * {depth}u + _p515_od]);"
    )
    kb.raw_line("                    }")
    kb.raw_line("                }")
    kb.raw_line("                _p515_maximum_a = _p515_newmax_a;")
    kb.raw_line("            }")
