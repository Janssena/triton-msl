...
def prepare(lowerer):
    ...
    uniform_ops = {
        ...
        'arith.remsi', 'arith.remui', 'arith.cmpi', 'arith.andi', 'arith.ori',
        'arith.xori', 'arith.select', 'arith.extsi', 'arith.extui', 'arith.trunci',
        'arith.index_cast', 'tt.splat', 'tt.expand_dims', 'tt.broadcast',
        'ttg.convert_layout',
    }
    ...
    def uniform(value, seen=frozenset()):
        ...
        if op.op in ('tt.get_program_id', 'tt.get_num_programs'):
            return True
        return (op.op in uniform_ops and bool(op.operand_ids)
                and all(uniform(v, seen | {value}) for v in op.operand_ids))
    ...
    control(graph.ops)
    allowed = {
        'tt.assert', 'tt.load', 'tt.store', 'tt.addptr', 'tt.make_range',
        'tt.splat', 'tt.broadcast', 'tt.expand_dims', 'ttg.convert_layout',
        'tt.get_program_id', 'tt.get_num_programs', 'tt.reduce', 'tt.reduce.return',
        'tt.return', 'tt.bitcast', 'tt.fp_to_fp', 'tt.precise_divf', 'tt.precise_sqrt',
        'tt.extern_elementwise', 'scf.for', 'scf.if', 'scf.yield',
        'ttg.barrier', 'tt.atomic_rmw', 'tt.atomic_cas',
    }
    for op in ops:
...
def emit(lowerer, op):
    ...
    lowerer.kb.raw_line(f'        atomic_fetch_max_explicit((device atomic_uint*)_assert_status, {code}u, memory_order_relaxed);')
    lowerer.kb.raw_line('    }')
