...
| atomics (`tt.atomic_rmw` / `tt.atomic_cas`) | ✓ / ⚠ | add/max/min/xchg/cas, incl. fp16/bf16 add; some 16-bit-float rmw ops refused (#5); n>1-per-thread atomic scatter refused (#13). **CAS contract:** MSL offers only the *weak* compare-exchange, which may fail spuriously; `tl.atomic_cas` is emitted as the strong emulation (retry while the reloaded value still equals `cmp`, compared as bits), so a spurious failure can never report "swapped" with nothing stored. **Ordering:** source `acquire` / `release` / `acq_rel` operations use scope-matched fences around the whole relaxed atomic (including CAS retry loops), on supported Metal 3.2+ targets; `sys` scope and missing/unknown metadata refuse. See [ordering contract](ATOMIC_ORDERING.md). **Returns:** every lane's old value on tensor atomics (N up to 1024, any `num_warps`); the return on *masked-off* lanes is 0 here (upstream leaves it unspecified — do not consume it). |
...
| `while` inside a `noinline` device function | ✗ refused | the device-function emitter lowers `scf.for` / `scf.if`, not `scf.while` (kernel-level `while` is fine) |
| **biased / triangle attention — forward** (trifast's `_biased_tri_fa` shape: `scores = q·kᵀ·scale + bias`, a per-column boolean mask selecting a sentinel, `exp2` online softmax, `lse` stored) | ✓ | `BLOCK_M = 32`, head_dim ≤ 192, 3-D grid `(j-block, triangle-i, batch·heads)`, bias shared across the triangle axis, cross-head mask via `pid_h // H`. Routed to the simdgroup-MMA kernel when head_dim %8 == 0 and (≤ 64 or 128 / 192) with contiguous head-dim strides, else the scalar tiled template (any strides, any `BLOCK_N`). **fp32, fp16, bf16**: the template replays the *source's* rounding points for narrow dtypes (`q * full(scale, dtype)`, `P.to(dtype)` per kv block relative to that block's running max, the output cast) — measured bf16 bit-exact against the source's online loop on the sampled rows; fp16 is NOT bit-exact: the template evaluates `exp` where the source evaluates `exp2` on log2-scaled scores, so a rare P tie flips (≤ one fp16 ulp of P on < 1 % of outputs in the sampled rows — a measured bound, not a proved one); a P-rounding source with a kv block ≠ 64 takes the tiled template (exact, slower). The dense (non-biased) tiled fp16 route's P rounding is still an open investigation, not covered by this row. Refused: a store mask tighter than the `N_CTX` tile boundary, an unproven score sentinel, ambiguous mask / bias roles, any value-path arithmetic the template does not replay. |
| **biased / triangle attention — backward** (trifast's `_bwd_kv` → dK / dV, `_bwd_q` → dQ + delta, `_bwd_b` → dbias) | ✓ / ⚠ | `BLOCK_J = BLOCK_K = 32`; simdgroup-MMA makers for head_dim %8 == 0 and ≤ 32, scalar k- / j-subtiled makers above that; dQ and dbias refuse at head_dim ≥ 128 (the J-independent K/V staging exceeds 32 KB); 3-D grid required. Every value path is *proved* (orientation calculus over the whole downstream graph, root identity of each operand, the exact loop recurrence, the reduce combiner's returned value) — a source that differs from the template's algebra refuses by name. **fp32**: exact. **fp16 / bf16**: dK / dV and dbias replay the source's rounding points (`k · dtype(scale)`, `P.to(dtype)` before the dV dot only, `dS.to(dtype)` before the dK dot, output casts). dQ replays them too — but trifast's unmodified `_bwd_q` sums `delta = tl.sum(o * do)` *in* the input dtype (a bf16 / fp16 reduction tree, an order-dependent rounding no fp32 template can replay bit-for-bit) and is **refused by name** on narrow inputs; the one-line variant `tl.sum((o * do).to(tl.float32), 1)` rounds each product (order-independent), sums in fp32, and computes. Score / lse round-trips through a narrow dtype are refused (not a rounding point of the source). |
| FlashAttention | ✓ (`BLOCK_M=BLOCK_N=32`, head_dim ∈ {32, 64, 128}) / ✗ (otherwise) | at **BLOCK_M = BLOCK_N = 32**: head_dim 32 (generic lowering) and **head_dim 64 + 128** routed to an Apple **`simdgroup_matrix` MMA kernel** (fp32 + fp16, causal + non-causal, any N_CTX) — dispatched **zero-copy** through `compile_shader`, **faster than PyTorch SDPA in every measured case** (fp16 full 1.65–1.99×, fp32 full 1.27–1.53×, causal up to ~4×) and **~0.88× Apple MLX** at the largest sizes; MLA (nope/rope) auto-routes. A trainable Metal **backward** pass (`triton_msl.fa_backward`) and **KDA** linear/delta attention (`triton_msl.kda`) ship as direct ops (see `docs/attention_ops.md`). Contiguity gate: non-contiguous innermost stride falls back to the scalar template. **Refused loudly** outside supported configs: `head_dim > 128`, **`BLOCK_M`/`BLOCK_N` ≠ 32** (the `<32` small-block case silently mis-computed for *any* head_dim — a hole the old head_dim>64 guard missed, closed 2026-06-17), and **bf16** matmul inputs (rejected at the Triton frontend / not routed). Larger blocks and head_dim > 128 are roadmap |
...
20. **A function body with more than one block** (`cf.br` chains in direct TTGIR) — the walker
    models single-block bodies; it refuses naming the block count rather than dropping the second
    block's ops. **Two public functions** in one module refuse likewise. (A function whose *first*
    op owns a region — a reduction-first callee, a `while`-first kernel — is walked correctly: the
    entry block is identified by region depth, not by the first op visited.)
21. **Biased-attention backward dQ with a narrow-dtype delta reduction** — see the backward row
    above: `tl.sum(o * do)` on fp16 / bf16 inputs accumulates in that dtype; refused with the
    one-line fp32-accumulation spelling in the message.
22. **MLX route (`triton_msl.mlx.triton_call`) boundaries** — the launcher binds arguments
    positionally against the parsed kernel signature and lets MLX allocate *fresh* outputs, so it
    refuses, before any dispatch: a kernel whose lowering set a dispatch descriptor the torch.mps
    driver honours (`flash_attention`, `mm_two_kernel`, `fast_matmul`, `quant_matmul`,
    `batched_dot_bounds`); an MSL holding more than one kernel; a parsed signature whose argument
    count differs from the Triton signature; an output pointer the kernel also reads or updates
    atomically (in-place / accumulating / atomic outputs — MLX outputs are uninitialised); an
    `mx.array` dtype outside {float32, float16, bfloat16, int8/16/32, uint8/16/32, bool}; a Python
    int outside int32.
