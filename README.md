...
> **0.3.0 — validation scope and limitations.** The numerical correctness, installed-acceptance and performance campaigns used the named 0.3.0rc4 artifacts. Final-version 0.3.0 artifacts have been freshly built and rebound to their tested runtime payloads, with fresh and relocated CPU checks. Historical throughput claims remain unqualified except for the explicitly scoped current attention comparisons below; those comparisons clear none of the main-relative performance alerts. The reported AlphaFold/trifast workload is not validated end-to-end, no independent Apple Silicon validation has run (one M4 Max only), and the optional C++ route is unaudited/deferred and off by default—leaving it off or packaging its sources is not validation. See `CHANGELOG.md` and the release-facing limitations at https://github.com/bledden/triton-msl/blob/main/docs/RELEASE_CANDIDATE_LIMITATIONS.md.

> Exact871 evidence: 6,079 project passes / 16 skips; unchanged 9,342-node upstream map (5,780 passed / 3,562 skipped); installed acceptance 1,468 native / 252 pure with no failures/skips; four M4 Max floors. Native and pure warm comparisons retain 13 and 14 open >10% alerts respectively. KDA prefill is a separate 1.1016× alert (block range 1.0721–1.1313). Cold first-specialization observations remain substantially slower (add 12.07 s vs 0.73 s native; compiled MLP 18.17 s vs 1.04 s) and include compilation, live observation and process residency—not whole-app startup. These remain open performance regressions, not universal model slowdowns or proof that further improvement is impossible. Wrong/refusing main receives no ratio; varlen uses a changed retained route and is not a same-route speedup.
...
> **Half-accumulate warning:** the shipped maker docstring's historical “~4% faster”, “~1% max-abs error” and “~0.01%” figures are unvalidated for exact871 and are not universal error bounds. The mode is opt-in and off by default; validate actual inputs/shapes. Default-float rescale tests provide no credit for that opt-in mode. Runtime text is unchanged.
...
The historical counts below describe the earlier published tree. Exact871 results and remaining limits are recorded in `docs/RELEASE_CANDIDATE_LIMITATIONS.md`.
