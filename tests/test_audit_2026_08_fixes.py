...
def test_biased_fa_tighter_output_store_mask_refuses():
    dev = "mps"; Z, H, N, DIM = 1, 2, 32, 64
    torch.manual_seed(0)
    sm = 1.0 / math.sqrt(DIM)
    q = torch.randn(Z, H, N, DIM, device=dev); k = torch.randn(Z, H, N, DIM, device=dev)
    v = torch.randn(Z, H, N, DIM, device=dev); b = torch.randn(Z, H, N, N, device=dev)
    o = torch.zeros(Z, H, N, DIM, device=dev); lse = torch.zeros(Z, H, N, device=dev)
    st = lambda t: t.stride()
    with pytest.raises(MetalNonRecoverableError):
        _biased_tighter_store[(triton.cdiv(N, 32), Z * H)](
            o, *st(o), lse, *st(lse), q, *st(q), k, *st(k), v, *st(v), b, *st(b),
            sm, Z, H, N, DIM, 32, 32)


# ------------------------- 2. int4 GEMV nibble pinning -----------------------
@triton.jit
def _int4_gemv_highfirst(x_ptr, w_ptr, o_ptr, s_ptr, z_ptr, N, K, ng, swn, ssn,
                         BN: tl.constexpr, BK: tl.constexpr, G: tl.constexpr):
    pid = tl.program_id(0); on = pid * BN + tl.arange(0, BN); ok = tl.arange(0, BK)
    ...
    for k in range(0, K, BK):
        ...
        g = kk // G
        s = tl.load(s_ptr + on[:, None] * ssn + g[None, :]); z = tl.load(z_ptr + on[:, None] * ssn + g[None, :])
        acc += tl.sum(tl.load(x_ptr + kk)[None, :] * ((w4.to(tl.float32) - z) * s), axis=1)
    tl.store(o_ptr + on, acc)


@triton.jit
def _int4_gemv_doubleswap(x_ptr, w_ptr, o_ptr, s_ptr, z_ptr, N, K, ng, swn, ssn,
                          BN: tl.constexpr, BK: tl.constexpr, G: tl.constexpr):
    pid = tl.program_id(0); on = pid * BN + tl.arange(0, BN); ok = tl.arange(0, BK)
    acc = tl.zeros((BN,), dtype=tl.float32)
    for k in range(0, K, BK):
...
def _run_int4_gemv(kernel):
    dev = "mps"; N, K, G = 128, 256, 128; ng = K // G
    torch.manual_seed(0)
    w4 = torch.randint(0, 16, (N, K), device=dev, dtype=torch.int32)
    packed = ((w4[:, 0::2] << 4) | w4[:, 1::2]).to(torch.uint8).contiguous()
    x = torch.randn(K, device=dev); s = (torch.rand(N, ng, device=dev) * 0.02 + 0.005).contiguous()
    z = torch.randint(0, 16, (N, ng), device=dev).float().contiguous(); o = torch.zeros(N, device=dev)
...
def _fa_nonlength_scalar(
    ...
    q = tl.load(Q + off_z * sqz + off_h * sqh + offs_m[:, None] * sqm + offs_d[None, :] * sqk,
                mask=offs_m[:, None] < valid_len, other=0.0)
    ...
    tl.store(Out + off_z * soz + off_h * soh + offs_m[:, None] * som + offs_d[None, :] * sok,
             acc.to(Out.dtype.element_ty), mask=offs_m[:, None] < valid_len)
...
def test_bwd_kv_head_dim256_refuses_cleanly():
    ...
    dev = "mps"; Hc, Hh, I, N, DIM = 1, 1, 1, 32, 256
    BJ = BK = 32; NEG = -1e9
