...
def _graph(dtype, bm, bn, causal):
    ...
    constants = dict(Z=1, stride_qk=1, stride_kk=1, stride_vk=1, stride_ok=1,
                     BLOCK_M=bm, BLOCK_N=bn, HEAD_DIM=64, IS_CAUSAL=causal)
    signature = {n: ('*'+dtype if n in ('Q', 'K', 'V', 'Out') else 'i32')
                 for n in _flash_attn_fwd.arg_names if n not in constants}
...
def test_larger_attention_tiles_compute_source(bm, bn, causal, dtype, executed, monkeypatch):
    ...
    scores = (q.float() * d**-.5) @ kp.transpose(-1, -2)
    if causal:
    ...
    def launch():
        return _flash_attn_fwd[(triton.cdiv(n, bm), 2)](
            ...
            1, 2, n, BLOCK_M=bm, BLOCK_N=bn, HEAD_DIM=d, IS_CAUSAL=causal)

    compiled = launch()
    torch.mps.synchronize()
    assert 'Head-dim-tiled FlashAttention-2' in _executed_source(executed, compiled)
    torch.testing.assert_close(out.cpu(), reference, atol=.002, rtol=.002)
...
def test_attention_does_not_discard_atomic_effects(block, effect, monkeypatch):
    ...
    update = ('tl.atomic_add(o_ptrs, acc.to(Out.dtype.element_ty), mask=offs_m[:, None] < N_CTX)'
              if effect == 'rmw' else 'tl.atomic_cas(o_ptrs, acc.to(Out.dtype.element_ty), tl.full((BLOCK_M, HEAD_DIM), 0., tl.float32))')
