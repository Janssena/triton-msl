...
@pytest.mark.parametrize('body,args,removed,live', [
    ...
    ('%x = arith.constant true\n%p = arith.xori %x, %x : i1\ntt.assert %p, "computed" : i1', '%out: !tt.ptr<i32>', 0, 1),
    ('%p = arith.constant dense<true> : tensor<8xi1, #b>\ntt.assert %p, "tensor" : tensor<8xi1, #b>', '%out: !tt.ptr<i32>', 0, 1),
    ('%p = arith.constant dense<[true, false, true, true, true, true, true, true]> : tensor<8xi1, #b>\ntt.assert %p, "mixed tensor" : tensor<8xi1, #b>', '%out: !tt.ptr<i32>', 0, 1),
    ('%t = arith.constant true\n%f = arith.constant false\ntt.assert %t, "true" : i1\ntt.assert %f, "false" : i1', '%out: !tt.ptr<i32>', 1, 1),
    ('%t = arith.constant true\nscf.if %condition {\ntt.assert %t, "nested" : i1\n}', '%out: !tt.ptr<i32>, %condition: i1', 0, 0),
])
...
def test_forged_text_graph_and_message_do_not_certify_true(tmp_path):
    ctx, src, options = _module(tmp_path, '%p = arith.constant false\n// arith.constant true\ntt.assert %p, "true" : i1')
    graph = walk_ttgir(src.module, options)
    for op in graph.ops:
...
tt.return %x : i32
    ...
    ctx, src, options = _module(tmp_path, '%x = arith.constant 4 : i32\n%y = tt.call @callee(%x) : (i32) -> i32', extra=extra)
    graph = walk_ttgir(src.module, options)
    assert discharge_scalar_true(graph, src.module) is graph
...
def test_constant_true_executes_without_flag_false_still_stops(monkeypatch, shader):
    ...
    patch_live_singleton_method(monkeypatch, _get_compile_shader_runtime if shader == '1' else _get_utils,
                                'dispatch' if shader == '1' else 'launch', wrap)
