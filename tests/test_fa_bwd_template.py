...
def test_bwd_kv_matches_autograd(Hc, Hh, I, N, DIM):
    ...
    qs, ks, vs, bs, ms, ls, ds, dos, dks, dvs = (
        S(qd), S(kd), S(vd), S(bd), S(mask), S(lse), S(delta), S(do), S(dk), S(dv))
    arg_decls = [
    ...
    bindings = {
        ...
        "v_sz": _u(vs[0]), "v_sh": _u(vs[1]), "v_sn": _u(vs[2]), "v_sk": _u(vs[3]),
        "b_sz": _u(bs[0]), "b_sh": "0u", "b_sm": _u(bs[1]), "b_sn": _u(bs[2]),
        ...
        "do_sz": _u(dos[0]), "do_sh": _u(dos[1]), "do_sm": _u(dos[2]), "do_sk": _u(dos[3]),
        "dk_sz": _u(dks[0]), "dk_sh": _u(dks[1]), "dk_sn": _u(dks[2]), "dk_sk": _u(dks[3]),
        "dv_sz": _u(dvs[0]), "dv_sh": _u(dvs[1]), "dv_sn": _u(dvs[2]), "dv_sk": _u(dvs[3]),
        "H": _u(Hh), "N_CTX": _u(N), "scale": "arg_scale",
    }
    src = make_flash_attention_bwd_kv_kernel(
        DIM, BJ, BK, out_dtype="fp32", arg_decls=arg_decls, bindings=bindings,
        grid_3d=True, mask_batch_div=_u(Hh))
    lib = torch.mps.compile_shader(src)
    TPG = BK * DIM
    n_k = (N + BK - 1) // BK
    lib.flash_attention_bwd_kv(dk, dv, qd, kd, vd, bd, mask, lse, delta, do, float(sm),
                               threads=(n_k * TPG, I, Hc), group_size=(TPG, 1, 1))
...
def test_bwd_q_matches_autograd(Hc, Hh, I, N, DIM):
    ...
    qs, ks, vs, bs, ms, ls, os_, dos, dqs, dls = (
        S(qd), S(kd), S(vd), S(bd), S(mask), S(lse), S(o_det), S(do), S(dq), S(delta))
    arg_decls = [
    ...
    bindings = {
        ...
        "mask_sz": _u(ms[0]), "mask_sh": _u(ms[1]), "mask_sn": _u(ms[2]),
        "lse_sz": _u(ls[0]), "lse_sh": _u(ls[1]), "lse_sm": _u(ls[2]),
        "o_sz": _u(os_[0]), "o_sh": _u(os_[1]), "o_sm": _u(os_[2]), "o_sk": _u(os_[3]),
        "do_sz": _u(dos[0]), "do_sh": _u(dos[1]), "do_sm": _u(dos[2]), "do_sk": _u(dos[3]),
        "dq_sz": _u(dqs[0]), "dq_sh": _u(dqs[1]), "dq_sm": _u(dqs[2]), "dq_sk": _u(dqs[3]),
        "dlt_sz": _u(dls[0]), "dlt_sh": _u(dls[1]), "dlt_sm": _u(dls[2]),
        "H": _u(Hh), "N_CTX": _u(N), "scale": "arg_scale",
    }
    ...
    TPG = BJ * DIM
    n_j = (N + BJ - 1) // BJ
    lib.flash_attention_bwd_q(dq, delta, qd, kd, vd, bd, mask, lse, o_det, do, float(sm),
                              threads=(n_j * TPG, I, Hc), group_size=(TPG, 1, 1))
    torch.mps.synchronize()
...
def test_bwd_b_matches_autograd(Hc, Hh, N, DIM):
    ...
    S = lambda t: t.stride()
    qs, ks, vs, bs, ms, ls, ds, dos, dbs = (
        S(qd), S(kd), S(vd), S(bd), S(mask), S(lse), S(delta), S(do), S(db))
    arg_decls = [
    ...
    bindings = {
        ...
        "b_sh": _u(bs[0]), "b_sm": _u(bs[1]), "b_sn": _u(bs[2]),
        ...
        "do_sh": _u(dos[0]), "do_si": _u(dos[1]), "do_sm": _u(dos[2]), "do_sk": _u(dos[3]),
        "db_sh": _u(dbs[0]), "db_sm": _u(dbs[1]), "db_sn": _u(dbs[2]),
        "H": _u(Hh), "N_CTX": _u(N), "scale": "arg_scale",
    }
    ...
    n_j = (N + BJ - 1) // BJ
    n_k = (N + BK - 1) // BK
    lib.flash_attention_bwd_b(db, qd, kd, vd, bd, mask, lse, delta, do, float(sm),
                              threads=(n_j * TPG, n_k, Hc), group_size=(TPG, 1, 1))
    torch.mps.synchronize()
