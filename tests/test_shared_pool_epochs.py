...
def emit(body):
    return """kernel void probe(uint lid [[thread_index_in_threadgroup]]) {
...
def test_reuse_requires_unconditional_threadgroup_memory_boundary(boundary, allowed):
    aliases = {}
    result = _alias_shared_memory(emit(
        "a[lid] = 1;\nfloat old = a[127-lid];\n" + boundary + "\nb[lid] = old;"
    ), allocation_aliases=aliases)
...
def test_loop_local_barrier_cannot_prove_backedge_reuse():
    ...
    safe = body.replace("float next = b[127-lid];", "float next = b[127-lid];\nthreadgroup_barrier(mem_flags::mem_threadgroup);")
...
def test_overlapping_lifetimes_do_not_alias_across_barrier():
    result = _alias_shared_memory(emit("""a[lid] = 1;
        threadgroup_barrier(mem_flags::mem_threadgroup);
        b[lid] = a[lid];
...
def test_different_types_and_preprocessor_paths_do_not_alias():
    ...
    conditional = emit(body.replace("threadgroup_barrier", "#if ENABLE\nthreadgroup_barrier")
                       .replace(";\nb[lid]", ";\n#endif\nb[lid]"))
