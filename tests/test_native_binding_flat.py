...
def equivalent(left,right):
    ...
    if left[0]=='error':assert left[1:]==right[1:]
    else:
        a,b=left[1],right[1]
...
def check(args,names,sig,plan):
    equivalent(outcome(lambda:s.bind_arguments(args,names,sig,_plan=plan)),
...
def test_native_i32_exact_width_and_fresh_outputs(native,value):
    names=['p','n','BLOCK'];sig=dict(p='*fp32',n='i32',BLOCK='constexpr')
    plan=s.make_binding_plan(names,sig);pointer=torch.empty(1,device='cpu');args=(pointer,value,object())
    a=native.bind_flat(args,names,sig,plan);b=native.bind_flat(args,names,sig,plan)
    ...
    a[0].clear();assert b[0]
    check(args,names,sig,plan)


def test_random_flat_and_original_positions(native):
    rng=random.Random(770);p=torch.empty(2,device='cpu')
    for _ in range(500):
        names=[f'a{i}' for i in range(rng.randrange(1,45))]
        sig={n:rng.choice(['i32','i32','*fp32','constexpr']) for n in names}
        sig[names[-1]]='i32';plan=s.make_binding_plan(names,sig)
        args=tuple(rng.randrange(-(1<<31),1<<31) if sig[n]=='i32' else rng.choice([p,None]) if sig[n]=='*fp32' else object() for n in names)
...
def test_unadmitted_i32_uses_exact_python_refusal(native,value):
    names=['v'];sig={'v':'i32'};plan=s.make_binding_plan(names,sig)
    assert native.bind_flat((value,),names,sig,plan) is NotImplemented
    check((value,),names,sig,plan)


@pytest.mark.parametrize('ty,value',[('i8',128),('u32',1<<31),('i64',1<<40),('fp16',1.25),('fp32',-0.0),('fp64',1.0),('bf16',1.25)])
def test_changed_live_width_falls_back(native,ty,value):
    names=['v'];sig={'v':'i32'};plan=s.make_binding_plan(names,sig);sig['v']=ty
    assert native.bind_flat((value,),names,sig,plan) is NotImplemented
    check((value,),names,sig,plan)


def test_exact_tensor_property_mutation_no_native_prefix(native,monkeypatch):
    pointer=torch.empty(1,device='cpu');events=[]
    names=['p','n'];sig={'p':'*fp32','n':'i32'};plan=s.make_binding_plan(names,sig)
    assert native.bind_flat((pointer,128),names,sig,plan) is not NotImplemented
    def getter(self):events.append('get');sig['n']='i8';return lambda:0
    ...
    left=outcome(lambda:s.bind_arguments((pointer,128),names,sig,_plan=plan));assert events==['get']
    events.clear();sig['n']='i32'
    right=outcome(lambda:s.bind_arguments_with_plan((pointer,128),names,sig,plan));assert events==['get']
...
def test_custom_scalar_and_callback_order_not_replayed(native):
    ...
    class Pointer:
        @property
        def data_ptr(self):events.append('pointer');return lambda:0
    class Integer(int):
        def __eq__(self,other):events.append(('eq',other));return super().__eq__(other)
    names=['p','v'];sig=dict(p='*fp32',v='i1');plan=s.make_binding_plan(['i'],{'i':'i32'})
    args=(Pointer(),Integer(1))
    assert native.bind_flat(args,names,sig,plan) is NotImplemented and not events
    left=outcome(lambda:s.bind_arguments(args,names,sig,_plan=plan));before=list(events);events.clear()
    right=outcome(lambda:s.bind_arguments_with_plan(args,names,sig,plan));assert events==before and events.count('pointer')==1
    equivalent(left,right)


@pytest.mark.parametrize('name,replacement',[('int',tuple),('tuple',int),('type',lambda value:object),('isinstance',lambda *args:False),('getattr',lambda *args:None),('callable',lambda *args:False),('zip',lambda *args:iter(())),('enumerate',lambda *args:iter(()))])
def test_live_builtin_bindings_preserved(native,monkeypatch,name,replacement):
    names=['p','v'];sig=dict(p='*fp32',v='i32');plan=s.make_binding_plan(names,sig);args=(torch.empty(1,device='cpu'),1)
...
def test_missing_or_custom_map_key_declines_without_equality(native):
    events=[]
    class Key(str):
        def __eq__(self,other):events.append(other);return super().__eq__(other)
        __hash__=str.__hash__
    names=['v'];sig={Key('v'):'i32'};plan=s.make_binding_plan(names,sig)
    assert native.bind_flat((1,),names,sig,plan) is NotImplemented and not events
    check((1,),names,sig,plan)
    sig={};check((1,),names,sig,plan)


def test_mutated_packer_map_and_forged_plan_preserved(native):
    names=['v'];sig={'v':'i32'};plan=s.make_binding_plan(names,sig);events=[]
    ...
    check((1,),names,sig,wrong);assert events==[1,1]
    raw={'i32':(False,struct.Struct('<i').pack)};proxy,cap=native.make_parts(raw);owned=s._BindingPlan(proxy,cap)
...
def test_plan_descriptor_changes_and_binder_replacement(native,monkeypatch):
    names=['v'];sig={'v':'i32'};plan=s.make_binding_plan(names,sig);calls=[]
    def replacement(*args,**kwargs):calls.append('binder');return ('custom',)
...
def test_pointer_instance_override_and_subclass_getattribute(native):
    names=['p','v'];sig={'p':'*fp32','v':'i32'};plan=s.make_binding_plan(names,sig)
    p=torch.empty(1,device='cpu');p.data_ptr=lambda:0
...
def test_null_pointer_and_strong_leaf_ownership(native):
    ...
    p=torch.empty(1,device='cpu');ref=weakref.ref(p);result=native.bind_flat((p,1),names,sig,plan)
    del p;gc.collect();assert ref() is result[0][0]
    del result;gc.collect();assert ref() is None
...
def test_native_path_counters(native):
    ...
    with pytest.raises(TypeError):native.bind_flat()
...
def test_live_struct_reinitialization_preserves_payload_and_refusal(native,format):
    names=['p','n'];sig={'p':'*fp32','n':'i32'};plan=s.make_binding_plan(names,sig)
    args=(None,1);packer=plan.packers['i32'][1]
    assert native.bind_flat(args,names,sig,plan) is not NotImplemented
    ...
    assert native.bind_flat(args,names,sig,plan) is not NotImplemented
    check(args,names,sig,plan)
    with pytest.raises(struct.error):packer.__self__.__init__('invalid')
    check(args,names,sig,plan)
...
def test_gc_threshold_mutation_after_native_return_is_live_next_call(native,mutation):
    ...
    old_threshold=gc.get_threshold();was_enabled=gc.isenabled();witnessed=0
    try:
        for delta in range(1,9):
            gc.collect();gc.disable()
            names=['p','n'];sig={'p':'*fp32','n':'i32'}
            pointer=torch.empty(1,device='cpu');args=(pointer,1)
            plan=s.make_binding_plan(names,sig);events=[]
            initial_hits=native.statistics()['hits']
            def callback(phase,info):
                if phase=='start':
                    events.append(native.statistics()['hits']-initial_hits)
                    if mutation=='signature':sig['n']='i64'
                    elif mutation=='struct':plan.packers['i32'][1].__self__.__init__('<q')
                    else:pointer.data_ptr=None
...
def test_nested_live_names_and_large_arity_fallback(native):
    ...
    for args,names,sig in [(((None,7),),['v'],{'v':('*fp32','i32')}), ((2,3),['a','b'],{'a':'i32','b':'i32'}), (tuple(range(129)),[str(i) for i in range(129)],{str(i):'i32' for i in range(129)})]:
        names.reverse();check(args,names,sig,plan)


def test_build_is_explicit_and_unsupported_remains_python():
    source=Path(__file__).resolve().parents[1]/'setup.py';tree=ast.parse(source.read_text())
    node=next(n for n in tree.body if isinstance(n,ast.FunctionDef) and n.name=='binder_extension_enabled')
    scope={};exec(compile(ast.Module(body=[node],type_ignores=[]),str(source),'exec'),scope);select=scope[node.name]
    ...
    for version in [(3,13),(3,15)]:
        ...
        with pytest.raises(RuntimeError):select('1','cpython',version,'darwin','arm64',False)


def python_plan_shim():
    scope={}
    exec('def bind_arguments_with_plan(args, names, signature, plan):\n    return bind_arguments(args, names, signature, _plan=plan)',vars(s),scope)
    return scope['bind_arguments_with_plan']


def test_c_dispatch_preserves_python_call_protocol(native):
    oracle=python_plan_shim();names=['n'];sig={'n':'i32'};plan=s.make_binding_plan(names,sig)
    calls=[(((1,),names,sig,plan),{}), ((),dict(args=(1,),names=names,signature=sig,plan=plan)),
           (((1,),names),dict(signature=sig,plan=plan)), ((),{}),
           (((1,),names,sig,plan,5),{}), (((1,),names,sig,plan),dict(plan=plan)),
           (((1,),names,sig),dict(unexpected=1))]
    for args,kwargs in calls:
        equivalent(outcome(lambda:oracle(*args,**kwargs)),outcome(lambda:s.bind_arguments_with_plan(*args,**kwargs)))


def test_initial_unsupported_plan_remains_live_python(native):
    names=['n','f'];sig={'n':'i32','f':'fp32'};plan=s.make_binding_plan(names,sig)
...
def test_foreign_global_lookup_runs_once_at_python_fallback(native):
    oracle=python_plan_shim();names=['n'];sig={'n':'i32'};plan=s.make_binding_plan(names,sig);events=[]
    class Key(str):
        def __eq__(self,other):
            events.append('lookup');sig['n']='i8'
            return super().__eq__(other)
        __hash__=str.__hash__
    key=Key('bind_arguments');saved=vars(s).pop('bind_arguments')
    try:
        vars(s)[key]=saved
        left=outcome(lambda:oracle((128,),names,sig,plan));before=list(events)
        events.clear();sig['n']='i32'
        right=outcome(lambda:s.bind_arguments_with_plan((128,),names,sig,plan))
        assert events==before==['lookup']
        equivalent(left,right)
    finally:
        vars(s).pop(key);vars(s)['bind_arguments']=saved
...
def test_live_native_helper_opt_out_keeps_python_behavior(native,monkeypatch,mode):
    names=['n'];sig={'n':'i32'};plan=s.make_binding_plan(names,sig)
    if mode=='deleted':monkeypatch.delattr(s,'_binder_native')
    else:monkeypatch.setattr(s,'_binder_native',None if mode=='none' else object())
    before=native.statistics()
    check((1,),names,sig,plan)
    after=native.statistics()
...
def test_foreign_fallback_lookup_side_effects_are_not_replayed(native,mode):
    oracle=python_plan_shim();names=['n'];sig={'n':'i64'};plan=s.make_binding_plan(names,sig);events=[]
    global_saved=vars(s)['bind_arguments'];sentinel=object()
    builtin_saved=vars(builtins).get('bind_arguments',sentinel)
    def replacement(*args,**kwargs):return ('replacement',)
    class Key(str):
        def __eq__(self,other):
            events.append(other is oracle.__code__.co_names[0])
            if mode=='raise':raise LookupError('foreign key marker')
            if mode=='binder':
                vars(s).pop(key);vars(s)['bind_arguments']=replacement
            elif mode=='builtins':vars(builtins)['bind_arguments']=replacement
            return False
        __hash__=str.__hash__
    key=Key('bind_arguments')
    def clear_lookup():
        for existing in list(vars(s)):
            if existing is key or (type(existing) is str and existing=='bind_arguments'):vars(s).pop(existing)
        vars(builtins).pop('bind_arguments',None)
    try:
        clear_lookup();vars(s)[key]=global_saved
        left=outcome(lambda:oracle((1,),names,sig,plan));left_events=list(events)
        clear_lookup();vars(s)[key]=global_saved;events.clear()
        right=outcome(lambda:s.bind_arguments_with_plan((1,),names,sig,plan))
        assert left==right and events==left_events==[True]
    finally:
        clear_lookup();vars(s)['bind_arguments']=global_saved
        if builtin_saved is not sentinel:vars(builtins)['bind_arguments']=builtin_saved


def test_missing_fallback_nameerror_attributes(native,monkeypatch):
    oracle=python_plan_shim();names=['n'];sig={'n':'i64'};plan=s.make_binding_plan(names,sig)
    ...
    for fn in (oracle,s.bind_arguments_with_plan):
        with pytest.raises(NameError) as caught:fn((1,),names,sig,plan)
