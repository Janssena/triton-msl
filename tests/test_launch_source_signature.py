...
@pytest.mark.parametrize("source_sig,expected", [
...
def test_ast_partial_signature_does_not_insert_omitted_constexpr(controlled, signature, expected):
    src = SimpleNamespace(signature=signature, constants={(1,): 32},
                          fn=SimpleNamespace(arg_names=["X", "BLOCK", "v", "O"]))
...
def _run_irsource_scalar(tmp_path, monkeypatch, dtype, torch_dtype, value, *, allow_fast):
  ...
  tt.func public @scalar(%p: !tt.ptr<{dtype}>, %v: {dtype}) {{
    ...
    compiled = triton.compile(str(path), target=GPUTarget("metal", "apple-m4", 32),
...
@pytest.mark.parametrize("dtype,value,hex_bytes", [
    ("i1", False, "00"), ("i1", True, "01"), ("u1", 1, "01"),
    ("i8", -2, "fe"), ("u8", 254, "fe"),
    ("i16", -2, "feff"), ("u16", 65534, "feff"),
    ("i32", -2, "feffffff"), ("u32", 4294967294, "feffffff"),
    ("i64", 2, "0200000000000000"), ("i64", -2, "feffffffffffffff"),
    ("u64", (1 << 63) + 3, "0300000000000080"),
    ("f16", 1.5, "003e"), ("fp16", 1.00048828125, "003c"),
    ("fp16", 1.00146484375, "023c"), ("bf16", 1.5, "c03f"),
    ("bf16", 1.00390625, "803f"), ("bf16", 1.01171875, "823f"),
    ("f32", 1.5, "0000c03f"), ("fp32", -2, "000000c0"),
])
...
@pytest.mark.parametrize("dtype,value", [
    ("i8", 128), ("u8", -1), ("i64", 1 << 63), ("u64", 1 << 64),
    ("i32", 1.5), ("i1", 2), ("fp64", 1.5), (None, 1), ("unknown", 1),
])
...
@pytest.mark.parametrize("dtype,value,safe", [
    ("fp32", 1.5, True), ("fp32", 2, False), ("fp32", True, False),
    ("i32", 1.5, False), ("i64", 2, True), ("i8", 128, False), ("u8", -1, False),
])
def test_fast_scalar_route_checks_python_representation(dtype, value, safe):
    launcher = SimpleNamespace(arg_names=["o", "v"], constexpr_indices=set(),
                               signature={"o": "*fp32", "v": dtype})
...
def test_tuple_constexpr_host_binding_copies_the_actual_output(tmp_path, monkeypatch):
    ...
    torch.mps.synchronize()
    print("TUPLE_SIGNATURE", compiled.src.signature, "OUTPUT_INDICES", compiled.packed_metadata[4],
          "ERR", (out - (x + 2)).abs().max().item(), flush=True)
    assert len(calls) == 1
