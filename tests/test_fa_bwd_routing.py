...
def _bwd_q(
    ...
    mask_ptrs = base_mask_ptr + (start_i * stride_maskm) + (k_idxs * stride_maskn)

    base_d_ptr = d_ptr + (start_h * stride_dh) + (start_i * stride_dm)
    d_ptrs = base_d_ptr + (j_idxs * stride_dn)

    base_dq_ptr = dq_ptr + (start_h * stride_dqh) + (start_i * stride_dqm)
    dq_ptrs = base_dq_ptr + (j_idxs[:, None] * stride_dqn) + (d_idxs[None, :] * stride_dqd)

    base_do_ptr = do_ptr + (start_h * stride_doh) + (start_i * stride_dom)
    do_ptrs = base_do_ptr + (j_idxs[:, None] * stride_don) + (d_idxs[None, :] * stride_dod)

    base_o_ptr = o_ptr + (start_h * stride_oh) + (start_i * stride_om)
    o_ptrs = base_o_ptr + (j_idxs[:, None] * stride_on) + (d_idxs[None, :] * stride_od)

    mask_j = j_idxs < N

    q_block = tl.load(q_ptrs, mask_j[:, None])
    sm_denom = tl.load(l_ptrs, mask_j)
    do_block = tl.load(do_ptrs, mask_j[:, None])
    o_block = tl.load(o_ptrs, mask_j[:, None])
    ...
    for start_k in range(0, N, BLOCK_K):
        ...
        b_block = tl.load(b_ptrs, mask_j[:, None] & mask_k[None, :]).to(tl.float32)
        m_block = tl.load(mask_ptrs, mask_k, cache_modifier=".cg")
        k_block = tl.load(k_ptrs, mask_k[:, None])
        k_block = k_block * tl.full([1], value=sm_scale, dtype=input_dtype)

        scores = tl.dot(q_block, tl.trans(k_block), b_block, input_precision="ieee")
        scores = tl.where(m_block[None, :], neg_inf, scores)
        scores = tl.where(mask_j[:, None] & mask_k[None, :], scores, neg_inf)

        sm_value = tl.math.exp2((scores - sm_denom[:, None]) * inv_ln2)

        vt_block = tl.load(vt_ptrs, mask_k[None, :])
        dsm_value = tl.dot(do_block, vt_block, input_precision="ieee")
