...
def test_biased_lowering_descriptor_matches_shader(bn):
    ...
    cex = {fn.arg_names[i]: (bn if fn.arg_names[i] == "BLOCK_K" else 32
                           if fn.arg_names[i] in ("DIM", "BLOCK_J") else 256)
           for i in fn.constexprs}
    sig = {n: ("*u8" if "mask" in n or n == "m_ptr" else
               "*fp32" if n.startswith("lse") else "*bf16") if n.endswith("_ptr")
...
def test_tiled_finite_scores_cover_tail(dtype, bn, causal, strided, n):
    ...
    src = makers.make_flash_attention_kernel_tiled(
        d, 32, bn, Dc=d, causal=causal,
        out_dtype={torch.float32: "f32", torch.float16: "f16", torch.bfloat16: "bf16"}[dtype],
    )
    lib = torch.mps.compile_shader(src)
    tpg = min(32 * bn, 1024)
    lib.flash_attention(q, k, v, out, *q.stride(), *k.stride(), *v.stride(), *out.stride(),
                        1, 1, n, threads=(math.ceil(n / 32) * tpg, 1), group_size=(tpg, 1))
    ...
    torch.testing.assert_close(out.float(), expected, atol={torch.float32: 2e-6, torch.float16: 1e-3, torch.bfloat16: 8e-3}[dtype], rtol=0)
    if strided:
