...
@pytest.mark.parametrize("ty,values", [
    ("i1", [False, True, 0, 1, 2]), ("u8", [0, 255, -1, 256]),
    ("i32", [1, -3, 1 << 31]), ("u64", [0, (1 << 63) + 3, 1 << 64]),
...
def test_custom_values_and_nested_declarations_preserve_original_behavior():
    ...
    for names, declared, args in [
        ...
        plan = signature.make_binding_plan(names, declared)
        assert signature.bind_arguments_with_plan(args, names, declared, plan) == signature.bind_arguments(args, names, declared)
...
def test_custom_scalar_fallback_never_replays_callbackful_prefix(monkeypatch):
    ...
    for planned in (False, True):
        ...
        with monkeypatch.context() as patch:
            ...
            result = (signature.bind_arguments_with_plan(args, names, declared, plan) if planned
...
def test_generic_dispatch_keeps_post_hook_and_submission_boundaries(monkeypatch, mode):
    ...
    class Runtime:
        ...
        def is_unsupported(self, _): return False
        def get_library(self, _): return object()
        def mark_unsupported(self, _): events.append("blacklist")
    ...
    def enter(_):
        events.append("enter")
        if mode == "hook_changes_type": launcher.signature["v"] = "i8"
    def leave(_):
        events.append("exit")
        if mode == "exit_hook_error": raise RuntimeError("after invocation")
    def call(): launcher(1, 1, 1, None, None, packed, None, enter, leave, pointer, 128)
    if mode == "hook_changes_type":
        with pytest.raises(HostFallback): call()
        assert events == ["enter"]
    elif mode in ("dispatch_error", "exit_hook_error"):
        with pytest.raises(PostSubmitError): call()
