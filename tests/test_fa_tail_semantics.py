...
def _tail_attention(
    ...
    FILL_K: tl.constexpr,
    FILL_V: tl.constexpr,
    PLAIN_K: tl.constexpr,
    PLAIN_V: tl.constexpr,
):
    pid = tl.program_id(0)
    zh = tl.program_id(1)
    z, h = zh // H, zh % H
    ...
    q = tl.load(Q + z * sqz + h * sqh + rm[:, None] * sqm + rd[None, :] * sqk, rm[:, None] < N_CTX, 0.0)
    ...
    for j in range(START, hi, STEP):
        ...
        m = mn
    tl.store(
        Out + z * soz + h * soh + rm[:, None] * som + rd[None, :] * sok,
        (acc / l[:, None]).to(Out.dtype.element_ty),
        rm[:, None] < N_CTX,
    )


def _graph(
    dtype, dim, mask, start=0, step=32, fill_k=0.0, fill_v=0.0, plain_k=False, plain_v=False, n_ctx_constant=None
):
    ...
    signature = {
        ...
        for n in _tail_attention.arg_names
...
def test_dense_tail_preserves_source(dtype, dim, mask, n, plain, monkeypatch, executed):
    ...
    graph, opts = _graph(
        dtype, dim, mask, plain_k=plain == "K", plain_v=plain == "V", n_ctx_constant=1 if n == 1 else None
    )
    lower = GenericLowerer(graph, opts)
    msl = lower.lower()  # Cached JIT execution cannot hide this contract.
    route_marker = (
        "Head-dim-tiled FlashAttention-2" if dim == 128 or dtype == "bf16" else "simdgroup-MMA FlashAttention-2"
    )
    ...
    if plain != "V":
        vp[:, :, n:] = 0
    scores = (q.float() * (dim**-0.5)) @ kp.transpose(-1, -2)
    if mask == 1:
        scores[:, :, :, n:] = -float("inf")
    elif mask == 3:
    ...
    tolerance = 0.02 if dtype == "bf16" else 0.002
    torch.testing.assert_close(out.cpu(), ref, atol=tolerance, rtol=tolerance)


@pytest.mark.parametrize(
    "change", [dict(start=32), dict(step=64), dict(fill_k=1.0), dict(fill_v=1.0), dict(mask=4), dict(mask=5)]
)
...
@pytest.mark.parametrize("term", ["kn[:, None]*skn", "kn[:, None]*svn", "rm[:, None]*sqm", "rm[:, None]*som"])
...
def test_dense_causal_loop_visits_exact_source_rows(dim, full, monkeypatch, executed):
    ...
    s = (q[:, :, :32] * dim**-0.5) @ k[:, :, :32].transpose(-1, -2)
    ...
    if not full:
        torch.testing.assert_close(got[:, :, :32], finite_ref, atol=0.002, rtol=0.002)
