...
def _desc(strides=None):
    ...
    strides = strides or (("arg", 5, "i32"), ("literal", 1, "i64"),
                          ("arg", 4, "i32"), ("literal", 1, "i64"),
                          ("arg", 4, "i32"), ("literal", 1, "i64"))
...
def test_negative_stride_and_storage_offset_are_proved_by_interval():
    ...
    a = a_base[64:]
    strides = (("literal", -64, "i64"), ("literal", 1, "i64"),
               ("arg", 4, "i32"), ("literal", 1, "i64"),
               ("arg", 4, "i32"), ("literal", 1, "i64"))
...
def test_aligned_k_does_not_observe_m_n_or_stride_scalars():
    ...
    assert driver._matmul_tail_bounds_reason(
        desc, [object(), object(), object(), NoInt(), NoInt(), 64] + [NoInt()] * 6) is None
...
def test_guard_scalars_must_match_captured_launch_payload():
    ...
    payloads = [None, None, None] + [
        _launch_signature.scalar_bytes(value, "i32") for value in args[3:]
    ]
...
def test_enter_hook_mutation_is_checked_before_runtime(monkeypatch):
    ...
    sigs = ["*fp32", "*fp32", "*fp32", "i32", "i32", "i32"]
    payloads = [None, None, None] + [
        _launch_signature.scalar_bytes(value, "i32") for value in flat[3:]
    ]
...
@pytest.mark.parametrize("address_bounds", [
...
def test_sealed_unknown_or_malformed_address_contract_refuses_before_runtime(address_bounds, monkeypatch):
    metadata = dict(
        name="sealed_bad_bounds", execution_contract="checked", num_warps=4, num_ctas=1,
        shared=0, block_size=128, output_arg_indices=None, needs_2d_grid=False,
        mm_two_kernel=None, fast_matmul=None, quant_matmul=None, flash_attention=None,
        batched_dot_bounds=address_bounds, device_assert=None,
    )
...
def test_both_valid_address_contract_tags_reach_their_proofs(monkeypatch):
    ...
    batched = (
        "batched_dot_host_bounds_v1", 1, 2, 2, 2, False, 0, 1, 2,
        (("literal", 4), ("literal", 2), ("literal", 1)),
        (("literal", 4), ("literal", 2), ("literal", 1)),
        (("literal", 4), ("literal", 2), ("literal", 1)),
    )
    assert driver._batched_dot_host_bounds_reason(
        batched, [torch.empty(4), torch.empty(4), torch.empty(4)], (1, 1, 1)) is None

    def launch_to_proof(address_bounds, args, sigs, payloads, expected, rejected, monkeypatch):
        metadata = dict(
            name="sealed_valid_bounds", execution_contract="checked", num_warps=4, num_ctas=1,
            shared=0, block_size=128, output_arg_indices=None, needs_2d_grid=False,
            mm_two_kernel=None, fast_matmul=None, quant_matmul=None, flash_attention=None,
            batched_dot_bounds=address_bounds, device_assert=None,
        )
    ...
    tail_payloads = [None, None, None] + [
        _launch_signature.scalar_bytes(value, "i32") for value in tail_args[3:]
    ]
    launch_to_proof(_desc(), tail_args, tail_sigs, tail_payloads,
                    "_matmul_tail_bounds_reason", "_batched_dot_host_bounds_reason", monkeypatch)
    launch_to_proof(batched, [torch.empty(4)] * 3, ["*fp32"] * 3, [None] * 3,
                    "_batched_dot_host_bounds_reason", "_matmul_tail_bounds_reason", monkeypatch)
