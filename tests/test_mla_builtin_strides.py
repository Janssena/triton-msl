...
def load(root, reference=False):
    ...
    exec(
        compile(
            ast.Module(body=[n for n in sub.body if isinstance(n, ast.ClassDef)], type_ignores=[]), "submission", "exec"
        ),
        scope,
    )
...
def exercise(scope, mode="normal", profile=False):
    ...
    try:
        ...
        try:
            value = scope["_dispatch_mla"](
                RT(),
                desc,
                args,
                grid=(2, 2, 1) if mode == "wrong_grid" else (1, 2, 1),
                launch_exit_hook=lambda m: event("exit"),
            )
