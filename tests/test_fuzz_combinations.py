...
def _run_mm_epi(rng):
    ...
    mm = A.float() @ B.float()
    ref = {0: mm * 2.0 + 1.0, 1: mm.clamp(min=0), 2: mm * 3.0 + 0.5, 3: mm.clamp(-1.0, 1.0)}[op]
...
def _run_splat_reduce(rng):
    N = rng.choice([8, 16, 32, 64, 128, 256])
    v = float(rng.choice([1.0, 2.0, 3.0]))
...
def _run_cat_ew(rng):
    ...
    PAD = 64
    obuf = torch.full((2 * N + PAD,), _SENT, device="mps")
    o = obuf[: 2 * N]
    _cat_ew[(1,)](a, b, o, N=N)
...
def _run_reduce_bcast(rng):
    R = rng.choice([1, 4, 8])
    C = rng.choice([8, 16, 32, 64, 128])
    BC = 1 << (C - 1).bit_length()
    torch.manual_seed(rng.randrange(1 << 30))
    a = torch.randn(R, C, device="mps")
    o = torch.empty(R, C, device="mps")
