...
@requires_gpu
@pytest.mark.parametrize("n,num_warps", [(32, 1), (8, 1), (256, 1), (256, 4), (1024, 4)],
...
def test_tensor_rmw_returns_every_lanes_old_value(cold_gpu_caches, n, num_warps):
    x0 = _x0(n); x = x0.clone(); out = torch.full((n,), -99, device=D, dtype=torch.int32)
...
def test_masked_rmw_return_on_inactive_lanes_is_the_zero_initializer(cold_gpu_caches):
    ...
    n, k = 32, 20
    x0 = _x0(n); x = x0.clone(); out = torch.full((1,), -99, device=D, dtype=torch.int32)
...
def test_float_max_returns_old_values_across_the_sign_split(cold_gpu_caches):
    n = 32
    x0 = (_x0(n).float() - 3) * 0.25; x = x0.clone(); out = torch.full((n,), -99.0, device=D)
...
def test_one_address_returns_n_distinct_old_values(cold_gpu_caches):
    n = 32
    x = torch.zeros(n, device=D, dtype=torch.int32); out = torch.full((n,), -99, device=D, dtype=torch.int32)
