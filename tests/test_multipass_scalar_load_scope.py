...
def _module(fn, signature, constants):
    ...
    mod = src.make_ir(backend.target, options, backend.get_codegen_implementation(options),
...
def test_scalar_load_survives_real_reduction_loop(monkeypatch):
    ...
    mod, options = _module(_scalar_scale, {'X': '*fp32', 'S': '*i64', 'O': '*fp32', 'MASK': 'i1'},
                           {'N': 2048})
...
def test_scalar_hoist_does_not_cross_effect_or_invent_scalar_shape(tmp_path, case):
    prefix = {'store': 'tt.store %o, %other : !tt.ptr<i64>',
              'assert': 'tt.assert %m, "keep guarded load behind assertion" : i1',
              'atomic': '%a = tt.atomic_rmw add, acq_rel, gpu, %o, %other, %m : (!tt.ptr<i64>, i64, i1) -> i64'}.get(case, '')
...
def test_scalar_mask_and_value_compute(mask, monkeypatch, tmp_path, executed):
    ...
    x = torch.linspace(-2., 2., 2048, device='mps')
    scale = torch.tensor([17], dtype=torch.int64, device='mps')
    output = torch.full((2052,), 123., device='mps')
    ...
    expected = expected * torch.rsqrt(expected.square().mean()+1.e-6) * (17 if mask else 3)
    actual = output.cpu()
    torch.testing.assert_close(actual[:2048].double(), expected, atol=1.e-5, rtol=1.e-5)
    assert torch.equal(actual[2048:], torch.full((4,), 123.))
