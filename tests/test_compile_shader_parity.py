...
def test_vadd_parity(flag, monkeypatch):
    ...
    _vadd[(triton.cdiv(N, 1024),)](A, B, OUT, N, BLOCK=1024)
...
def test_reduction_parity(flag, monkeypatch):
    monkeypatch.setenv("TRITON_MSL_COMPILE_SHADER", flag)
    R, N = 32, 512
    X = torch.randn(R, N, device="mps")
    OUT = torch.empty(R, device="mps")
    _row_sum[(R,)](X, OUT, N, BLOCK=triton.next_power_of_2(N))
    torch.mps.synchronize()
...
def test_softmax_parity(flag, monkeypatch):
    ...
    R, N = 16, 256
    X = torch.randn(R, N, device="mps")
    OUT = torch.empty(R, N, device="mps")
    _softmax[(R,)](X, OUT, N, BLOCK=triton.next_power_of_2(N))
    torch.mps.synchronize()
...
def test_where_parity(flag, monkeypatch):
    ...
    OUT = torch.empty(N, device="mps")
    _where[(triton.cdiv(N, 1024),)](A, B, COND, OUT, N, BLOCK=1024)
    torch.mps.synchronize()
...
def test_atomic_add_parity(flag, monkeypatch):
    ...
    _scatter_add[(triton.cdiv(N, 1024),)](IDX, VAL, OUT, N, BLOCK=1024)
    torch.mps.synchronize()
...
def test_2d_grid_parity(flag, monkeypatch):
    ...
    _add2d[(triton.cdiv(N, 1024), M)](A, B, OUT, M, N, BLOCK=1024)
    torch.mps.synchronize()
    torch.testing.assert_close(OUT, A + B, rtol=1e-4, atol=1e-4)
