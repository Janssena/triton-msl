...
@triton.jit
def _epilogue_mm(A, B, Bias, C, M, N, K, SA, SB, SC, alpha, beta, BM: tl.constexpr, BN: tl.constexpr, BK: tl.constexpr):
    pid = tl.program_id(0)
    nt = tl.cdiv(N, BN)
...
def compile_case(dtype, bm, bn, bk):
    sig = {
        n: "*" + dtype if n in ("A", "B", "Bias", "C") else "fp32" if n in ("alpha", "beta") else "i32"
        for n in _epilogue_mm.arg_names
        if n not in ("BM", "BN", "BK")
    }
...
def test_epilogue_runtime(dtype, bm, bn, bk, monkeypatch):
    ...
    out = torch.full((mp, np), -2048.0, device="mps", dtype=dt)
    calls = []
    ...
    h = _epilogue_mm[(4,)](
        a.to("mps"), b.to("mps"), bi.to("mps"), out, m, n, k, k, np, np, -0.5, 1.25, BM=bm, BN=bn, BK=bk
    )
    ...
    assert torch.equal(actual[m:, :], torch.full_like(actual[m:, :], -2048.0))
    assert torch.equal(actual[:m, n:], torch.full_like(actual[:m, n:], -2048.0))
...
def test_wide_epilogue_nonfinite_column_and_finite_siblings(monkeypatch):
    ...
    h = _epilogue_mm[(1,)](a, b.to("mps"), bias, out, 32, 64, 64, 64, 64, 64, -0.5, 1.25, BM=32, BN=64, BK=32)
    torch.mps.synchronize()
    assert "Source-replayed" in h.asm.get("msl", h.asm.get("metal", ""))
    actual = out.cpu()
