...
SPEC = importlib.util.spec_from_file_location('release_artifacts', Path(__file__).resolve().parents[1] / 'scripts/release_artifacts.py')
...
class ReleaseArtifacts(unittest.TestCase):
    def setUp(self):
        ...
        self.source.mkdir(); self.dist.mkdir()
        self.stem = 'triton_msl-0.3.0rc4'
        self.files = {'pyproject.toml': b'[project]\nversion="0.3.0rc4"\n',
                      'LICENSE': b'MIT notice', 'third_party/trifast/LICENSE': b'trifast notice',
        ...
        for mode, tag in [('native', 'cp314-cp314-macosx_15_0_arm64'), ('pure', 'py3-none-any')]:
            ...
            payload[info + 'WHEEL'] = f'Wheel-Version: 1.0\nRoot-Is-Purelib: {str(mode == "pure").lower()}\nTag: {tag}\n'.encode()
            payload[info + 'METADATA'] = b'Name: triton-msl\nVersion: 0.3.0rc4\n'
            if mode == 'native':
                for helper in release.HELPERS:
                    payload['triton_msl/backend/' + helper + '.cpython-314-darwin.so'] = struct.pack('<II', 0xfeedfacf, 0x0100000c)
            self.wheels[mode] = (self.dist / (self.stem + '-' + tag + '.whl'), payload)
        self.write()

    def write(self):
        with tarfile.open(self.dist / (self.stem + '.tar.gz'), 'w:gz') as archive:
            top = tarfile.TarInfo(self.stem); top.type = tarfile.DIRTYPE; archive.addfile(top)
            for name, data in self.sd.items():
                info = tarfile.TarInfo(self.stem + '/' + name); info.size = len(data)
    ...
    def test_wrong_binary_architecture(self):
        self.wheels['native'][1]['triton_msl/backend/_packed_native.cpython-314-darwin.so'] = struct.pack('<II', 0xfeedfacf, 0x01000007)
    ...
    def test_fresh_sources_and_explicit_native_modes(self):
        ...
        with patch.object(release, 'native_platform'), patch.object(release.subprocess, 'run', side_effect=build), \
