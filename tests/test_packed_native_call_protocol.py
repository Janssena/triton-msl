...
pytestmark = pytest.mark.skipif(c.PACKED_COPY_IMPLEMENTATION != 'native',
                                reason='requires the optional native copier')
...
def test_repeated_copies_own_every_nested_container():
    value = [[7], {'stride': [1, 32]}]
    plan = (list, ((list, ((int, 7),)),
                   (dict, (('stride', (list, ((int, 1), (int, 32)))),))))
...
@pytest.mark.parametrize('name,value,plan,provider', [
