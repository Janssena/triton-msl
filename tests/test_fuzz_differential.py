...
def _mm(a, b, c, M, N, K, sam, sak, sbk, sbn, scm, scn, BM: tl.constexpr, BN: tl.constexpr, BK: tl.constexpr):
    ...
    for k0 in range(0, K, BK):
        kk = k0 + rk
        a_ = tl.load(a + rm[:, None] * sam + kk[None, :] * sak, mask=(rm[:, None] < M) & (kk[None, :] < K), other=0.0)
        b_ = tl.load(b + kk[:, None] * sbk + rn[None, :] * sbn, mask=(kk[:, None] < K) & (rn[None, :] < N), other=0.0)
        acc += tl.dot(a_, b_)
    tl.store(
...
def _ew(a, b, o, n, OP: tl.constexpr, BLOCK: tl.constexpr):
    i = tl.arange(0, BLOCK)
    m = i < n
    x = tl.load(a + i, mask=m, other=0.0)
    y = tl.load(b + i, mask=m, other=0.0)
    if OP == 0:
        z = x + y
    elif OP == 1:
        z = x * y
...
def _run_matmul(rng):
    ...
    BM = rng.choice(_POW2)
    BN = rng.choice(_POW2)
    BK = rng.choice([8, 16, 32])
    A = torch.randn(M, K, device="mps", dtype=dt)
    B = torch.randn(K, N, device="mps", dtype=dt)
...
def _run_single_dot(rng):
    ...
    out = rng.choice(_DTYPES)
    S = rng.choice(_POW2)
    A = torch.randn(S, S, device="mps", dtype=dt)
    B = torch.randn(S, S, device="mps", dtype=dt)
    PAD = 64
    Cbuf = torch.empty(S * S + PAD, device="mps", dtype=out)
    Cbuf[S * S :] = _SENT
    C = Cbuf[: S * S].view(S, S)
    _single_dot[(1,)](A, B, C, S=S)
    canary = int((Cbuf[S * S :] != _SENT).sum().item())
    return C.float(), A.float() @ B.float(), canary, max(_TOL[dt], _TOL[out]), f"single_dot {dt}->{out} S{S}"


def _run_reduce(rng):
    dt = rng.choice(_DTYPES)
    op = rng.choice([0, 1, 2])
    scale = rng.choice([False, True])
    R = rng.choice([1, 4, 8, 16])
    C = rng.choice([7, 8, 16, 31, 32, 64, 100, 128])
    BC = 1 << (C - 1).bit_length()  # smallest pow2 >= C
    a = torch.randn(R, C, device="mps", dtype=dt)
...
def _run_ew(rng):
    dt = rng.choice(_DTYPES)
    op = rng.choice([0, 1, 2, 3, 4])
    n = rng.choice([7, 8, 31, 32, 100, 128, 257, 1000])
    BLOCK = 1 << (n - 1).bit_length()
    a = torch.randn(n, device="mps", dtype=dt)
    b = torch.randn(n, device="mps", dtype=dt)
    o = torch.empty(n, device="mps", dtype=dt)
    _ew[(1,)](a, b, o, n, OP=op, BLOCK=BLOCK)
    af, bf = a.float(), b.float()
    ref = {0: af + bf, 1: af * bf, 2: torch.maximum(af, bf), 3: torch.where(af > bf, af, bf), 4: torch.exp(af)}[op]
...
def _run_scan(rng):
    ...
    dt = torch.float32
    BLOCK = rng.choice([8, 32, 256, 1024, 2048])
...
def _run_atomic(rng):
    ...
    BLOCK = 1 << (n - 1).bit_length()
    a = torch.randn(n, device="mps", dtype=dt)
    o = torch.zeros(K, device="mps", dtype=dt)
...
def test_fuzz(seed):
    ...
    try:
        ...
        pytest.fail(f"[seed {seed}] cryptic crash (not a clean refusal): {type(e).__name__}: {str(e)[:200]}")
...
def test_transpose_within_threadgroup_correct():
    ...
    torch.manual_seed(0)
    a = torch.randn(S, S, device="mps")
    o = torch.empty(S, S, device="mps")
    _trans_sq[(1,)](a, o, S=S)
    torch.mps.synchronize()
...
def test_truncf_scalar_quantizes():
    ...
    a = torch.zeros(4, device="mps")
    a[:3] = torch.tensor([2049.0, 4097.0, 100.0])
...
def test_gather2d_within_threadgroup_correct():
    for M, N in [(4, 8), (8, 16), (16, 32), (32, 32)]:
        ...
        idx = torch.randint(0, M, (M, N), dtype=torch.int32, device="mps")
        out = torch.empty(M, N, dtype=torch.float32, device="mps")
        _gather2d_pin[(1,)](src, idx, out, M=M, N=N)
        torch.mps.synchronize()
        ref = torch.gather(src.cpu(), 0, idx.cpu().long())
        torch.testing.assert_close(out.cpu(), ref, rtol=1e-4, atol=1e-4), f"2D gather M={M} N={N} SILENT-WRONG"
...
def test_cat_within_threadgroup_correct(fn):
    ...
    PAD = 64
    obuf = torch.full((2 * N + PAD,), float("nan"), device="mps")
    obuf[2 * N :] = 1234.0
    o = obuf[: 2 * N]
    fn[(1,)](a, b, o, N=N)
    torch.mps.synchronize()
