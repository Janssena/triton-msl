...
@pytest.mark.parametrize('change', ['parent', 'parent-replace', 'path', 'path-replace',
                                  'dictionary', 'epoch', 'finder', 'instance-method', 'name'])
...
@pytest.mark.parametrize('name', ['__iter__', '__len__', '_recalculate', '_get_parent_path',
...
'''
    ...
    result = subprocess.run([sys.executable, '-c', script, mode, root], env=env,
                            cwd='/private/tmp', capture_output=True, text=True, timeout=30)
...
def test_canonical_method_comparison_checks_constants_without_callbacks():
    ...
    changed = original.replace(co_consts=tuple('/different' if type(item) is str else item
