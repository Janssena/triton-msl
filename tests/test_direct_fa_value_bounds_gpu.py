...
@pytest.mark.parametrize(
    "qk,v,dtype",
    [
        (128, 32, torch.float16),
        (128, 40, torch.float16),
        (128, 96, torch.float16),
        (192, 96, torch.float16),
        (128, 32, torch.float32),
        (128, 40, torch.float32),
...
def test_direct_value_width_matches_reference_and_preserves_output_canaries(qk, v, dtype, causal, n):
    ...
    source = make_flash_attention_kernel_simdgroup(
        head_dim=qk,
        v_head_dim=v,
        out_dtype="fp16" if dtype == torch.float16 else "fp32",
        causal=causal,
        kernel_name="direct_value_bounds",
    )
