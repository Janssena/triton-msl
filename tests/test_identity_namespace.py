...
@pytest.mark.parametrize('kind', ['paths', 'list-subclass', 'tuple-subclass', 'truth-error',
                                  'iteration-error', 'policy-side-effect', 'module-getter',
                                  'module-getattr', 'spec-getter', 'spec-descriptor'])
    ...
    for mode in ('full', 'fast'):
        ...
        with monkeypatch.context() as m:
            ...
            if kind in ('paths', 'truth-error', 'iteration-error', 'policy-side-effect'):
                ...
                m.setattr(spec, 'submodule_search_locations', ListPaths(paths) if kind == 'list-subclass' else TuplePaths(paths))
