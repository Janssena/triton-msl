...
def test_structural_nctx_routes_and_computes(Z, H, N, D, causal):
    ...
    _fa_renamed[(triton.cdiv(N, BM), Z * H)](
        q, k, v, o, *q.stride(), *k.stride(), *v.stride(), *o.stride(), Z, H, N, BM, BN, D, causal
    )
...
def _fa_ambiguous(
    ...
    q = tl.load(
        Q + off_z * sqz + off_h * sqh + offs_m[:, None] * sqm + offs_d[None, :] * sqk,
        mask=(offs_m[:, None] < seqlen) & (offs_m[:, None] < other_bound),
        other=0.0,
    )
    q = q * (1.0 / tl.sqrt(float(HEAD_DIM)))
    m_i = tl.full([BLOCK_M], float("-inf"), dtype=tl.float32)
    ...
    for start_n in range(0, seqlen, BLOCK_N):
        k = tl.load(
            K + off_z * skz + off_h * skh + (start_n + offs_n)[:, None] * skn + offs_d[None, :] * skk,
            mask=(start_n + offs_n)[:, None] < other_bound,
            other=0.0,
        )
        qk = tl.dot(q, tl.trans(k).to(q.dtype))
        m_ij = tl.max(qk, 1)
