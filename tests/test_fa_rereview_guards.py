...
@triton.jit
def _dense_fa_split_scale(Q, K, V, O, sqz, sqh, sqm, sqk, skz, skh, skn, skk,
...
@triton.jit
def _varlen_store_shift(Q, K, V, Out, cu_q, cu_k,
    ...
    sm = tl.program_id(0); bh = tl.program_id(1); b = bh // H; h = bh % H
    h_out = (h + 1) % H                            # writes the NEXT head
    qs = tl.load(cu_q + b); slq = tl.load(cu_q + b + 1) - qs
    ks = tl.load(cu_k + b); slk = tl.load(cu_k + b + 1) - ks
    om = sm * BM + tl.arange(0, BM); on = tl.arange(0, BN); od = tl.arange(0, D)
    q = tl.load(Q + (qs + om)[:, None] * sqt + h * sqh + od[None, :] * sqd, mask=om[:, None] < slq, other=0.) * SCALE
    mi = tl.full([BM], float("-inf"), tl.float32); li = tl.zeros([BM], tl.float32); acc = tl.zeros([BM, D], tl.float32)
    for sn in range(0, max_seqlen, BN):
        kn = sn + on
        k = tl.load(K + (ks + kn)[:, None] * skt + h * skh + od[None, :] * skd, mask=kn[:, None] < slk, other=0.)
        qk = tl.where(kn[None, :] < slk, tl.dot(q, tl.trans(k).to(q.dtype)), float("-inf"))
        m2 = tl.maximum(mi, tl.max(qk, 1)); a = tl.exp(mi - m2); p = tl.exp(qk - m2[:, None])
        li = li * a + tl.sum(p, 1); acc = acc * a[:, None]
        vv = tl.load(V + (ks + kn)[:, None] * svt + h * svh + od[None, :] * svd, mask=kn[:, None] < slk, other=0.)
        acc += tl.dot(p.to(tl.float32), vv.to(tl.float32)); mi = m2
    tl.store(Out + (qs + om)[:, None] * sot + h_out * soh + od[None, :] * sod,
...
def test_varlen_store_head_shift_not_misrouted():
    ...
    T = int(cu[-1]); scale = 1.0 / math.sqrt(D)
...
def _bwd_kv_vshift(
    ...
    sm_scale, neg_inf, N, H, NH, DIM: tl.constexpr,
    ...
    vt_ptrs = v_ptr + (start_h_v * stride_vh) + (start_i * stride_vm) + (d_idxs[:, None] * stride_vd) + (k_idxs[None, :] * stride_vn)
...
def test_bwd_v_head_shift_refuses():
    ...
    sm = 1.0 / math.sqrt(DIM); BJ = BK = 32; NEG = -1e9
    q = torch.randn(Hc, I, N, DIM, device=dev); k = torch.randn(Hc, I, N, DIM, device=dev)
    v = torch.randn(Hc, I, N, DIM, device=dev); bias = torch.randn(Hc, N, N, device=dev)
    mask = (torch.rand(Hc // Hh, I, N, device=dev) < 0.15).to(torch.uint8)
    do = torch.randn(Hc, I, N, DIM, device=dev)
    lse = torch.randn(Hc, I, N, device=dev); delta = torch.randn(Hc, I, N, device=dev)
    dk = torch.zeros(Hc, I, N, DIM, device=dev); dv = torch.zeros(Hc, I, N, DIM, device=dev)
