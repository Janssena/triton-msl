...
def _invariant(run, reference, dtype, *, must=None):
    ...
    try:
        ...
        pytest.fail(f"cryptic crash (not a clean MetalNonRecoverableError): {type(e).__name__}: {str(e)[:200]}")
    if must == "ref":
        pytest.fail("backend SILENTLY COMPUTED a config that must refuse loudly (silent-wrong risk)")
    ref = reference()
    denom = max(ref.float().abs().max().item(), 1e-9)
    rel = (out.float() - ref.float()).abs().max().item() / denom
    assert rel < _TOL[dtype], f"SILENT-WRONG: rel_err {rel:.3e} exceeds tol {_TOL[dtype]} with NO refusal"
    return "correct"
...
def _mm_kloop(a, b, c, M, N, K, BM: tl.constexpr, BN: tl.constexpr, BK: tl.constexpr):
    ...
    for k in range(0, K, BK):
        acc += tl.dot(
            tl.load(a + om[:, None] * K + (k + ok)[None, :]), tl.load(b + (k + ok)[:, None] * N + on[None, :])
        )
...
def test_matmul_kloop(dtype, M, N, K):
    ...
    _invariant(
        lambda: (_mm_kloop[(triton.cdiv(M, BM), triton.cdiv(N, 32))](A, B, C, M, N, K, BM=BM, BN=32, BK=32), C)[1],
...
def test_matmul_single_dot_no_kloop(dtype, out_dtype, S):
    ...
    assert canary_bad == 0, (
        f"OOB WRITE: single-dot {dtype}->{out_dtype} S={S} clobbered {canary_bad}/{PAD} bytes past the output buffer"
    )
    ref = A.float() @ B.float()
...
def test_matmul_single_dot_rect(dtype, M, N, K):
    ...
    C = Cbuf[: M * N].view(M, N)
    try:
        _mm_single_rect[(1,)](A, B, C, M=M, N=N, K=K)
        torch.mps.synchronize()
    except MetalNonRecoverableError:
        return
    except Exception as e:
...
def test_flash_attention_dtype(dtype, head_dim):
    ...
    def ref():
        return torch.softmax((q.float() * (1.0 / math.sqrt(D))) @ k.float().transpose(-2, -1), -1) @ v.float()
...
def test_cumsum(dtype, BLOCK):
    ...
    _invariant(lambda: (_cumsum[(1,)](a, o, BLOCK=BLOCK), o)[1], lambda: a.float().cumsum(0), dtype)
