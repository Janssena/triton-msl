...
def case(monkeypatch):
    launcher = SimpleNamespace(_msl='arbitrary source already validated by stash', _msl_block_size=64,
...
@pytest.mark.parametrize('kind', ['group_mismatch','group_clamp','no_stash_group','template','packing',
    'z','zero_grid','float_grid','overflow_grid','flat_grid','cpp','unknown_policy',
    'dtype','subclass','noncontiguous','empty','half_scalar','high_u64','aggregate'])
def test_unproved_forms_retain_host_eligibility(case,kind):
    launcher,args,sigs,meta,grid=case
    if kind=='group_mismatch':launcher._msl_block_size=32
    elif kind=='group_clamp':launcher._msl_block_size=meta[3]=2048
    elif kind=='no_stash_group':launcher._msl_block_size=None
    elif kind=='template':meta[6]={'special':True}
    elif kind=='packing':args*=11;sigs*=11
    elif kind=='z':grid=(3,5,2)
    elif kind=='zero_grid':grid=(0,5,1)
    elif kind=='float_grid':grid=(3.,5,1)
    elif kind=='overflow_grid':grid=(1<<30,5,1)
    elif kind=='flat_grid':meta[5]=False
    elif kind=='cpp':launcher._execution_contract=json.dumps({'source':{'policy':{'USE_CPP':True}}})
    elif kind=='unknown_policy':launcher._execution_contract='{}'
    elif kind=='dtype':args[0]=torch.zeros(960,dtype=torch.int32)
    elif kind=='subclass':args[0]=torch.nn.Parameter(args[0])
    elif kind=='noncontiguous':args[0]=torch.zeros(1920)[::2]
    elif kind=='empty':args[0]=torch.zeros(0)
    elif kind=='half_scalar':sigs[2]='fp16';args[2]=1.0
    elif kind=='high_u64':sigs[2]='u64';args[2]=1<<63
    elif kind=='aggregate':sigs[2]=('i32','i32');args[2]=(1,2)
    assert generic_2d_geometry(launcher,args,sigs,meta,grid) is None


@pytest.mark.parametrize('declared,value', [('i8',-128),('u8',255),('i16',-32768),('u16',65535),
    ('i32',-(1<<31)),('u32',(1<<32)-1),('i64',1<<40),('i64',-(1<<63)),('u64',(1<<63)-1),('fp32',1.25)])
...
def test_public_launcher_keeps_validation_and_no_replay(case,monkeypatch,mode):
    ...
    events=[];flags=[]
    class HostFallback(Exception):pass
    class Utils:
        @property
        def buffer_pool(self):events.append('host');raise HostFallback
    class Runtime:
        def available(self):return True
        def is_unsupported(self,s):return False
        def mark_unsupported(self,s):events.append('blacklist')
        def get_library(self,s):
            if mode=='compile_miss':raise RuntimeError('pre invocation')
            return object()
        def dispatch(self,lib,name,actual,**geometry):
            events.append('dispatch');flags.append(actual[-1])
            ...
            if mode=='assertion':actual[-1].fill_(1)
            if mode=='dispatch_error':raise RuntimeError('attempted')
    ...
    def zeros(*a,**kw):
        if kw.get('device')=='mps':kw['device']='cpu'
        return original_zeros(*a,**kw)
    monkeypatch.setattr(torch,'zeros',zeros)
    for obj,name in ((_cache_contract,'validate_execution_contract'),(_launch_contract,'validate_packed_launch'),
                     (_launch_signature,'bind_arguments_with_plan')):
        original=getattr(obj,name)
        def record(*a,_fn=original,_name=name,**kw):events.append(_name);return _fn(*a,**kw)
        monkeypatch.setattr(obj,name,record)
    launcher=driver.MetalLauncher.__new__(driver.MetalLauncher);launcher.__dict__.update(vars(stub))
    launcher.arg_names=['a','b','n'];launcher.signature=dict(zip(launcher.arg_names,sigs));launcher.constexpr_indices=set()
    launcher._binding_plan=_launch_signature.make_binding_plan(launcher.arg_names,launcher.signature)
    launcher._packed_contract=_launch_contract._canonical(meta);launcher.kernel_name='unrecognized_generic'
    def leave(_):
        if mode=='exit_error':raise RuntimeError('exit hook')
    if mode=='bad_scalar':args[2]=1<<40
    def call():launcher(*grid,None,None,meta,None,None,leave,*args)
    if mode=='clean':
        call();call();assert flags[0] is not flags[1]
    else:
        error={'assertion':MetalDeviceAssertionError,'dispatch_error':PostSubmitError,'exit_error':PostSubmitError,
               'compile_miss':HostFallback,'bad_scalar':MetalNonRecoverableError}[mode]
        with pytest.raises(error):call()
        ...
        else:assert 'dispatch' not in events
