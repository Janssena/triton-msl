...
def _gqa_reference(Hc, GROUP, Hh, I, N, DIM, seed=0):
    ...
    k_e = k.repeat_interleave(GROUP, dim=0)
    v_e = v.repeat_interleave(GROUP, dim=0)
    ...
    o = torch.einsum("hijk,hikd->hijd", p, v_e)
    o.retain_grad()
    o.backward(do)
...
def _bwd_kv_2d(
    ...
    input_dtype = q_ptr.dtype.element_ty
    pid_k = tl.program_id(0)
    pid_ih = tl.program_id(1)  # 2-D grid: i + head combined
    pid_i = pid_ih % I
    pid_h = pid_ih // I
    inv_ln2: tl.constexpr = 1.4426950408889634
    mask_start_h = pid_h // H
