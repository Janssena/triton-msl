...
def literal():
    return json.dumps(
        {"schema": 1, "source": cache.source_contract(), "toolchain": cache.toolchain_identity()},
        sort_keys=True,
        separators=(",", ":"),
    )
...
def test_single_capture_reuses_only_after_live_checks(inputs, monkeypatch):
    ...
    assert events == (
        ["framework/native", "environment"] if owned else ["framework/native", "environment", "environment"]
    )
...
def test_framework_side_effect_precedes_capture_and_foreign_mapping_stays_live(inputs, monkeypatch):
    ...
    class StatefulGetter(dict):
        ...
        def get(self):
            ...
            count = self.calls
            return (
                lambda key, default=None: ("1" if count % 2 else "0")
                if key == "TRITON_MSL_FA_FAST"
                else dict.get(self, key, default)
            )
...
def test_callbacks_select_remaining_providers_without_replaying_prefix(inputs, monkeypatch):
    ...
    for mode in (
        "framework_policy",
        "framework_compiler",
        "framework_native_compiler",
        "policy_compiler",
        "capture_compiler",
        "capture_changes_environment",
    ):
        results = []
