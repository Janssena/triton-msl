...
def test_retained_assert_debug_true_checks_before_access(monkeypatch, shader):
    ...
    out = torch.full((8,), -99.0, device="mps")
    ...
    assert torch.equal(out.cpu(), torch.full((8,), -99.0)), "guarded access/store ran after a failed assertion"
...
def _compile_guarded_callee(debug):
    ...
    retained += sum(o.op == "tt.assert" for f in (graph.called_funcs or []) for o in _walk(f.ops))
...
def _masked_loop_assert(TABLE, INDEX, OUT, active, iterations, B: tl.constexpr):
    ...
    for step in range(iterations):
        ...
        total += tl.load(TABLE + idx, lane < active, other=0.0)
...
def test_masked_loop_assert_group_stop_and_lane_semantics(monkeypatch, shader):
    ...
    out = torch.full((b,), -99.0, device="mps")
    ...
    for bad in (-1, 1000000):
        ...
        assert torch.equal(out.cpu(), torch.full((b,), -99.0)), "a threadgroup continued after its failed check"
    indices[33] = 33
    _masked_loop_assert[(1,)](table, indices, out, b - 1, 3, B=b, debug=True)
...
def test_lowering_boundary_real_asserts_precede_guarded_access():
    graph, lowerer = _entry_lowerer(
        _masked_loop_assert,
        {"TABLE": "*fp32", "INDEX": "*i32", "OUT": "*fp32", "active": "i32", "iterations": "i32"},
        {"B": 64},
    )
    ...
    assert (
        len(
            re.findall(
                r"bool (assert_failed_\d+) = atomic_load_explicit[^;]+;\s*threadgroup_barrier\(mem_flags::mem_threadgroup\);\s*if \(\1\) return;",
                msl,
            )
        )
        == 2
