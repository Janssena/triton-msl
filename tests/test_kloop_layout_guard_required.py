...
@triton.jit
def _mixed_fused(A, B, C, M, N, K, sam, sbk, scm, BM: tl.constexpr, BN: tl.constexpr, BK: tl.constexpr):
...
def test_mixed_fused_width_cannot_bypass_guard(tmp_path, monkeypatch):
    ...
    with pytest.raises(
        MetalNonRecoverableError, match="unmasked K-loop source address widths are unresolved; mask the K tail"
    ) as error:
        _emit("i64", {})
...
def test_existing_layout_guard_refuses_when_tail_plan_check_is_isolated(width, tmp_path, monkeypatch):
    ...
    if width == "i32":
        monkeypatch.setattr(GenericLowerer, "_dot_offset_arith_widths", lambda self: None)
    with pytest.raises(
        MetalNonRecoverableError, match="requires proven operand strides and source integer widths"
    ) as error:
        _emit(width, {})
    assert error.value.op_name == "tt.dot"
