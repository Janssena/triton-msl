...
def selection():
    ...
    node = next(n for n in tree.body if isinstance(n, ast.FunctionDef)
                and n.name == "packed_extension_enabled")
    namespace = {}
...
@pytest.mark.parametrize("implementation,version,system,machine,gil", [
...
def test_unsupported_auto_is_python_but_explicit_request_raises(selection, implementation, version, system, machine, gil):
...
def test_combined_build_modes_preserve_legacy_opt_out(validation, packed):
    ...
    outputs = []
    namespace = dict(os=SimpleNamespace(environ=env),
    ...
    expected = []
    if effective_validation != '0':expected.append('triton_msl.backend._validation_native')
    if effective_packed != '0':expected.append('triton_msl.backend._packed_native')
    if effective_validation != '0':expected.append('triton_msl.backend._binder_native')
